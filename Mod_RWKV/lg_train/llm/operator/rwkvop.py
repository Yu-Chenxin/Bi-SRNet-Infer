...
if os.environ["WKV"] == 'fla':
    ...
    if 'x070' in os.environ["RWKV_MY_TESTING"]:
        ...
        load(name="wind_backstepping", sources=[f'lg_train/llm/cuda/wkv7_cuda.cu', 'lg_train/llm/cuda/wkv7_op.cpp'], is_python_module=False, verbose=True, extra_cuda_cflags=flags)
        ...
        if os.environ["RWKV_TRAIN_TYPE"] == 'infctx':
            wkv6state_cuda = load(name="wkv6infctx", sources=["lg_train/llm/cuda/wkv6infctx_op.cpp", f"lg_train/llm/cuda/wkv6infctx_cuda.cu"],
            ...
            wkv6state_cuda = load(name="wkv6state", sources=["lg_train/llm/cuda/wkv6state_op.cpp", f"lg_train/llm/cuda/wkv6state_cuda.cu"],
                            verbose=True, extra_cuda_cflags=["-res-usage", "--use_fast_math", "-O3", "-Xptxas -O3", "--extra-device-vectorization", f"-D_N_={HEAD_SIZE}", f"-D_T_={int(os.environ['RWKV_CTXLEN'])}"])
            ...
            wkv6_cuda = load(name="wkv6", sources=["lg_train/llm/cuda/wkv6_op.cpp", f"lg_train/llm/cuda/wkv6_cuda.cu"],
                            verbose=True, extra_cuda_cflags=["-res-usage", "--use_fast_math", "-O3", "-Xptxas -O3", "--extra-device-vectorization", f"-D_N_={HEAD_SIZE}", f"-D_T_={int(os.environ['RWKV_CTXLEN'])}"])
        ...
        wkv5_cuda = load(name="wkv5", sources=["lg_train/llm/cuda/wkv5_op.cpp", f"lg_train/llm/cuda/wkv5_cuda.cu"],
